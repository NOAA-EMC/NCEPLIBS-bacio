 ...
 * Fortran compiler                                                       
 *
 * @note In your Fortran code, call banio(), not banio_(). 
 *
 * What is going on here is that although the Fortran caller will
 * always be calling banio(), the called C routine name will change from
 * system to system.
 * 
 * @return 0 All was well                                   
 * @return 255 Tried to open read only _and_ write only       
...
/** 
 * Do banio operation, with new names for long int arguments, needed
 * for files > 2 Gb.
 *
 * @param mode integer specifying operations to be performed see the
